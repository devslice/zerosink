  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/resolve") {
      ...
      if (!env.STRIPE_SECRET_KEY) {
        return new Response(JSON.stringify({ error: "Stripe API key not configured on licensing server." }), {
          status: 500,
          headers
        });
      }

      const stripeSessionUrl = `https://api.stripe.com/v1/checkout/sessions/${sessionId}`;
      try {
        ...
        if (!subId) {
          return new Response(JSON.stringify({ error: "Subscription not found for this checkout session." }), {
            status: 404,
            headers
          });
        }
        ...
        return new Response(JSON.stringify({ error: `Connection failed: ${e.message}` }), {
          status: 500,
          headers
        });
      }
